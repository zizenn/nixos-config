...
class ScriptData {
  ...
  constructor(leafName, headerText, noExec, scriptType) {
    ...
    this.styleSheetMode =
      scriptType === ScriptType.STYLE
        ? headerText.match(/\/\/ @stylemode\s+(.+)\s*$/im)?.[1] ===
          "agent_sheet"
          ? "agent"
          : "author"
        : null;
    ...
    let windowActor =
      scriptType === ScriptType.SCRIPT
        ? headerText.match(/\/\/ @WindowActor\s+(.+)\s*$/im)?.[1]
        : null;
    this.actorMatches = windowActor
      ? headerText.match(/\/\/ @WindowActorMatches\s+(.+)\s*$/im)?.[1]
      : null;
    this.windowActor = this.actorMatches ? windowActor : null;
    if (
      this.inbackground ||
      this.styleSheetMode === "agent" ||
      (scriptType === ScriptType.SCRIPT && noExec)
    ) {
      this.regex = null;
      this.loadOrder = -1;
    } else {
  ...
  static buildScriptActorDefinition(aActorName, aMatches) {
    ...
    if (!matches.every((a) => typeof a === "string")) {
      throw new Error(
        `${aScript.filename}: WindowActorMatches for '${aActorName}' includes non-strings`,
      );
    }
    return {
      parent: {
...
function checkLoaderUpdateWhenIdle() {
  const idleService = Cc["@mozilla.org/widget/useridleservice;1"].getService(
  ...
  const updateCheckObserver = (subject, topic, data) => {
    if (topic === "idle") {
      ...
      checkLoaderUpdate({
        ignoreVersion: Services.prefs.getStringPref(ignoreVersionPref, ""),
        ignoreIfAlreadyChecked: true,
      });
    }
  };
...
class UserChrome_js {
  constructor() {
    ...
    this.EXPERIMENTS_ENABLED = Services.prefs.getBoolPref(
      PREF_EXPERIMENTAL,
      false,
    );
    ...
    if (aScript.windowActor) {
      if (this.EXPERIMENTS_ENABLED) {
        ...
        if (!/[A-Za-z][A-Za-z0-9_]*/.test(windowActor)) {
          console.warn(
            `${aScript.filename}: WindowActor name '${windowActor}' is not acceptable`,
          );
          return isDisabled;
        }
        if (aBuiltActorMap.has(windowActor)) {
          console.warn(
            `${aScript.filename}: WindowActor '${windowActor}' already in use `,
          );
          return isDisabled;
        }
        try {
          let def = ScriptData.buildScriptActorDefinition(
            windowActor,
            aScript.actorMatches,
          );
        ...
        console.warn(
          `fx-autoconfig: Script '${aScript.filename}' tries to use WindowActor - an experimental feature that is currently disabled.`,
        );
      }
    }
    return isDisabled;
  ...
  init() {
    ...
    if (this.EXPERIMENTS_ENABLED && windowActorDefinitions.size > 0) {
      lazy.ActorManagerParent.addJSWindowActors(
        Object.fromEntries(Array.from(windowActorDefinitions.entries())),
      );
    ...
    if (
      Services.prefs.getBoolPref(
        "userChromeJS.updates.update-check.enabled",
        false,
      )
    ) {
  ...
  generateScriptMenuItemsIfNeeded(aDoc) {
    ...
    Promise.all([
      ...
      L10n.formatMessage("update-checkForUpdatesButton"),
      ...
      aDoc
        ...
        .setAttribute("label", values[1].replace("…", "") + " & " + baseTitle);
      aDoc
        .getElementById("userScriptsMenu-OpenFolder")
        .setAttribute("label", values[2]);
      aDoc
        .getElementById("userScriptsMenu-CheckUpdate")
        .setAttribute(
          "label",
          updateMsg.attributes.find((a) => a.name === "label")?.value ||
            "Check for updates",
        );
    });
    ...
    if (aSubject.document.isUncommittedInitialDocument) {
      ...
      aSubject.addEventListener(
        ...
        () => {
          ...
          parent.addEventListener("DOMContentLoaded", this, {
