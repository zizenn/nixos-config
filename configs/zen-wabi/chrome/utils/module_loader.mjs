{
  ...
  let { loaderModuleLink } = ChromeUtils.importESModule(
    "chrome://userchromejs/content/utils.sys.mjs",
  );

  let disabledScripts = Services.prefs
    .getStringPref(PREF_SCRIPTSDISABLED, "")
    .split(",");

  let moduleScripts = loaderModuleLink.matchScripts(
    (s) =>
      s.isESM &&
  ...
  for (let script of moduleScripts) {
    import(script.chromeURI)
      .catch((ex) => {
        console.error(
          new Error(`@ ${script.filename}:${ex.lineNumber}`, { cause: ex }),
        );
