...
class Storage {
  ...
  keys() {
    return Object.getOwnPropertySymbols(this.#storage).concat(
  ...
  set(aKey, aValue) {
    ...
    if (this.#listeners.size > 0) {
      this.#changeset.set(key, {
        oldValue: this.#changeset.get(key)?.oldValue ?? oldValue,
        newValue: aValue,
      });
      if (this.#idleCallback === 0) {
        this.#idleCallback = lazy.requestIdleCallback(() =>
          this.#onIdleCallback(),
        );
      }
    }
...
class LoaderLink {
  ...
  get brandName() {
    if (this.#brandName === null) {
      this.#brandName = ChromeUtils.importESModule(
        "resource://gre/modules/AppConstants.sys.mjs",
  ...
  get loaderInfo() {
    if (!this.#loaderInfo) {
      let aFile = FileSystem.convertChromeURIToFileURI(
        Services.io.newURI(`chrome://userchromejs/content/boot.sys.mjs`),
      ).QueryInterface(Ci.nsIFileURL).file;
      let result = FileSystem.readNSIFileSyncUncheckedWithOptions(aFile, {
        metaOnly: true,
      });
      let headerText = extractScriptHeader(result);
      let info = this.#ScriptFactory(
        aFile.leafName,
        headerText,
        false,
        this.#ScriptType.LOADER,
        true,
      );
  ...
  createScriptInfo(aName, aStringAsFSResult, isStyle, isEnabled) {
    return this.#createScriptInfo(
      aName,
      aStringAsFSResult,
      isStyle ? this.#ScriptType.STYLE : this.#ScriptType.SCRIPT,
      isEnabled,
    );
  }
  #createScriptInfo(aName, aStringAsFSResult, type, isEnabled) {
    const headerText = extractScriptHeader(aStringAsFSResult);
    const scriptData = this.#ScriptFactory(
      aName,
      headerText,
  ...
  matchScripts(aFilter, uriOnly) {
    return LoaderLink.#getScriptInfoForType(
      aFilter,
      this.#scripts,
      uriOnly
        ? LoaderLink.#basicScriptInfo
        : LoaderLink.#scriptDataToScriptInfo,
    );
  }
  ...
  static #getScriptInfoForType(aFilter, aScriptList, mapFn) {
    ...
    if (filterType === "function") {
      return aScriptList
        .filter(aFilter)
        .map((script) =>
          mapFn(script, !disabledScripts.includes(script.filename)),
        );
    }
...
export class ScriptInfo {
  ...
  async checkScriptUpdate() {
    ...
    let scriptInfo = LoaderLink._cloneScriptInfo(
      loaderModuleLink,
      this,
      content,
      this.#scriptType,
    ...
    return {
      ...
      remoteVersion,
      available,
      script: { info: scriptInfo, response: content },
    };
  }
  static fromString(aName, aStringAsFSResult, isStyle) {
    return loaderModuleLink.createScriptInfo(
      aName,
      aStringAsFSResult,
      isStyle,
      false,
...
export class windowUtils {
  ...
  static getWindowChromeFlags(win) {
    ...
    return Object.fromEntries(
      Object.entries(Ci.nsIWebBrowserChrome).map(([flag, value]) => [
        flag,
        (windowFlags & value) === value,
      ]),
    );
  }
}
...
export function compareVersionString(ver1, ver2) {
  ...
  if (
    ![ver1, ver2].every((v) =>
      /^(0|[1-9][0-9]{0,8})([.](0|[1-9][0-9]{0,8})){0,3}$/.test(v),
    )
  ) {
  ...
  buffer.set(
    ver1.split(".").map((b) => Number.parseInt(b)),
    0,
  );
  buffer.set(
    ver2.split(".").map((b) => Number.parseInt(b)),
    4,
  );
...
export async function checkLoaderUpdate(opt = {}) {
  ...
  if (update.available && !(ignoreVersion === update.remoteVersion)) {
    ...
    let dlMsg =
      message.attributes.find((a) => a.name === "buttonlabel")?.value ||
      "Download";
    let dismissMsg =
      message.attributes.find((a) => a.name === "secondarybuttonlabel")
        ?.value || "Dismiss";
    showNotification({
